...
class Varien_Autoload
{
     ...
     * @throws Exception
     */
    public static function registerComposerAutoload()
    {
        $vendorDirectory = self::getVendorRootDirectory();
        $autoloadFileName = $vendorDirectory . DIRECTORY_SEPARATOR . 'autoload.php';
        if (!file_exists($autoloadFileName)) {
            throw new Exception('The composer autoload.php file was not found.');
        }

        return include $autoloadFileName;
    }

    /**
     * Retrieve vendor root directory
     *
     * @return bool|string
     */
    public static function getVendorRootDirectory()
    {
        if (getenv('MAGE_VENDOR_ROOT')) {
            return getenv('MAGE_VENDOR_ROOT');
        }

        if (defined('MAGE_VENDOR_ROOT')) {
            return MAGE_VENDOR_ROOT;
        }

        $basePath = self::getRootPath();
        $vendorDirName = self::getVendorDirectoryName();

        if (is_dir($basePath . 'lib' . DIRECTORY_SEPARATOR . $vendorDirName)) {
    ...
    /**
     * Retrieve vendor directory name
     *
     * @return string
     */
    public static function getVendorDirectoryName()
    {
        if (getenv('MAGE_VENDOR_DIR_NAME')) {
            return getenv('MAGE_VENDOR_DIR_NAME');
        }

        if (defined('MAGE_VENDOR_DIR_NAME')) {
            return MAGE_VENDOR_DIR_NAME;
        }
    ...
    /**
     * Retrieve Magento installation root directory
     *
     * @return string
     */
